...
/*
		...
		memcpy(pglobal->in[plugin_number].buf, pics->sequence[i].data, 
			pglobal->in[plugin_number].size);
*/
        i = (i+1) % LENGTH_OF(pic_arr);
		//printf("i=%d\n",i);
        pglobal->in[plugin_number].size = pic_arr[i].size;
		memcpy(pglobal->in[plugin_number].buf, pic_arr[i].data, 
			pglobal->in[plugin_number].size);
